...
- 合约没有直接使用第三方权限库，而是实现了轻量化权限控制。
- `owner` 负责管理授权名单，`authorizedUploaders` 负责限制写链地址范围。
- 这意味着任何地址都可以读链上数据，但只有被授权地址才能执行哈希写入、异常上链等敏感操作。

论文可写描述：

“智能合约采用双层权限模型，使用 `owner` 管理授权地址集合，并通过 `onlyAuthorized` 修饰器限制写链操作，从而保证链上写入行为仅由受信任的后台服务或管理地址执行。”
...
- 该段代码是链上防篡改能力的核心。
- 合约不直接存整份监控数据，而是只保存运单数据的摘要值 `bytes32 dataHashDigest`。
- `storeOrderHashDigest()` 负责写入，`verifyOrderHashDigest()` 负责校验，两者共同构成链上存证与校验机制。
- 同一运单只允许写入一次，避免链上快照被重复覆盖。

论文可写描述：

“为降低链上存储成本，系统仅将运单监控数据的摘要值写入智能合约，而不直接上传原始监控数据。通过摘要上链与摘要比对，可以在不暴露业务明文的前提下实现数据完整性验证。”
...
- `startAnomalyLite()` 用于在异常开始时写入轻量级异常记录。
- 为了降低 gas 开销，链上只保存异常摘要和关键字段，不直接保存完整异常明文。
- `closeAnomaly()` 在异常恢复后更新结束时间和峰值，使链上记录形成完整生命周期。
- 这种“开始上链 + 结束补全”的设计，更适合持续性异常场景。

论文可写描述：

“系统将异常事件拆分为开始与结束两个链上动作，在异常开始时记录触发值、起始时间和异常信息摘要，在异常结束时补充结束时间和峰值，从而完整反映异常生命周期，同时控制链上存储成本。”
